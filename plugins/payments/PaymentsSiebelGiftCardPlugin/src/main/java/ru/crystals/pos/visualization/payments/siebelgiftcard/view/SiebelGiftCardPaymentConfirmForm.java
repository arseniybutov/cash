...
public class SiebelGiftCardPaymentConfirmForm extends AbstractPaymentForm<
        PaymentEntity,
    ...
    public SiebelGiftCardPaymentConfirmForm(XListener outerListener) {
        super(outerListener);
    }
    ...
    public CommonPaymentHeaderPanel createHeaderPanel() {
        return new CommonPaymentHeaderPanel(
                ResBundlePaymentSiebelGiftCard.getString("GIFTCARD_PAYMENT"),
    ...
    public boolean keyPressedNew(XKeyEvent e) {
        if (e.getKeyCode() >= KeyEvent.VK_0 && e.getKeyCode() <= KeyEvent.VK_9 || e.getKeyCode() == KeyEvent.VK_COMMA ||
            ...
            /*
             * Проверим, что введенное число не больше суммы на карте или суммы к оплате
             * Проверим на ноль
             */
            ...
            if (amountFromPanel.compareTo(payment.getSumPayBigDecimal()) > 0 ||
                ...
                e.setSource("SiebelPaymentsPlugin: empty or zero summ field, cannot process card");
