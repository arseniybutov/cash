...
public class ExtBankTerminalPaymentEnterReceiptNumberForm extends AbstractPaymentForm<PaymentEntity, CommonPaymentHeaderPanel, ExtBankPayment2UnitPanel,
        CommonPaymentToPayPanel, CommonProductInputPanel, ExtBankTerminalPaymentController> {
    public ExtBankTerminalPaymentEnterReceiptNumberForm(XListener outerListener) {
        super(outerListener);
    }
    ...
    public CommonProductInputPanel createFooterPanel() {
        return new CommonProductInputPanel(CommonProductInputPanel.InputType.INTEGER, ResBundlePaymentExternalBankTerminal.getString("ENTER_CHECK_NUMBER"), "", 10);
    }

    @Override
    public ExtBankPayment2UnitPanel createLeftPanel() {
        return new ExtBankPayment2UnitPanel(new CommonPaymentUnitPanel(ResBundlePaymentExternalBankTerminal.getString("CARD_NUMBER"), ""),
                new CommonPaymentUnitPanel(ResBundlePaymentExternalBankTerminal.getString("AUTH_CODE"), ""));
    }
    ...
    public boolean keyPressedNew(XKeyEvent e) {
        if ((e.getKeyCode() >= KeyEvent.VK_0 && e.getKeyCode() <= KeyEvent.VK_9) || e.getKeyCode() == KeyEvent.VK_BACK_SPACE || Character.isDigit(e.getKeyChar())) {
            ...
            if (footerPanel.isClean()) {
                getController().processReceiptNumberCanceled();
                return true;
            } else {
                footerPanel.reset();
                return true;
            ...
            controller.processReceiptNumberEntered(footerPanel.getTextValue());
        }
        return false;
    }
