...
public class CFTEGCPaymentEnterCardNumberForm extends
        AbstractPaymentForm<PaymentEntity, CommonPaymentHeaderPanel, CommonPaymentPaidPanel, CommonPaymentToPayPanel, CommonProductInputPanel, CFTEGCPaymentController> {
    private static final long serialVersionUID = 1246960910712062632L;

    public CFTEGCPaymentEnterCardNumberForm(XListener outerListener) {
        super(outerListener);
    }

    @Override
    public CommonPaymentHeaderPanel createHeaderPanel() {
        return new CommonPaymentHeaderPanel(ResBundlePaymentCFTEGC.getString("CFT_GIFTCARD_PAYMENT"), ResBundlePaymentCFTEGC.getString("CFT_GIFTCARD_REFUND"));
    }
    ...
    public CommonProductInputPanel createFooterPanel() {
        return new CommonProductInputPanel(CommonProductInputPanel.InputType.CERT_NUMBER);
    }
    ...
    public String getEnteredNumber() {
        String result = footerPanel.getTextValue();
        return result != null ? result.replace(" ", "") : result;
    }
}
