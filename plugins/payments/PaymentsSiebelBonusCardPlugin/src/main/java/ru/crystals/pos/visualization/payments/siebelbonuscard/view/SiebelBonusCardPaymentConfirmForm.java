...
public class SiebelBonusCardPaymentConfirmForm extends
        AbstractPaymentForm<PaymentEntity, CommonPaymentHeaderPanel, CommonPaymentPaidPanel, CommonPaymentToPayPanel, CommonProductInputPanel,
                SiebelBonusCardPaymentController> {
    public SiebelBonusCardPaymentConfirmForm(XListener outerListener) {
        super(outerListener);
    }
    ...
    public CommonPaymentHeaderPanel createHeaderPanel() {
        return new CommonPaymentHeaderPanel(ResBundlePaymentSiebelBonusCard.getString("SIEBEL_BONUSCARD_PAYMENT"));
    }
    ...
    public boolean keyPressedNew(XKeyEvent e) {
        if (e.getKeyCode() >= KeyEvent.VK_0 && e.getKeyCode() <= KeyEvent.VK_9 || e.getKeyCode() == KeyEvent.VK_COMMA ||
            ...
            if (amountFromPanel.compareTo(payment.getSumPayBigDecimal()) > 0 ||
                ...
                e.setSource("SiebelBonusCardPayment: empty or zero summ field, cannot process card");
    ...
    public void showForm(PaymentEntity payment, PaymentInfo info) {
        ...
        summPanel.setPaymentSumm(getController().isRefund() ? payment.getSumPayBigDecimal() : info.getSurcharge());
