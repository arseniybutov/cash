 ...
 */
public class GetFieldStructureV2Command extends BaseCommand<FieldStructure> {
     ...
     */
    public GetFieldStructureV2Command(byte tableNo, byte fieldNo, int password) {
        super(password);
    ...
    public FieldStructure decodeResponse(byte[] response) {
        ...
        if(ShtrihFieldType.NUMBER == fieldType) {
    ...
    public boolean validateResponse(byte[] response) {
        ...
        int fieldWidth = response[45];
        if(ShtrihFieldType.STRING == fieldType) {
            fieldWidth = 1;
        }
