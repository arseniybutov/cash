...
/**
 * Команда "Выплата" (Изъятие).
 * <p/>
 * {@link #decodeResponse(byte[]) Ответом} данной команды будет СПНД (Сквозной порядковый номер документа) зарегистрированного документа [изъятия].
 * 
 * @author aperevozchikov
 */
public class CashOutCommand extends BaseCommand<Integer> {

    /**
     * Сумма изъятия, в МДЕ (минимальных денежных единицах) - в "копейках".
     */
    private long sum;
     ...
     */
    public CashOutCommand(long sum, int password) {
        super(password);
        this.sum = sum;
    }
    ...
    public String toString() {
        return String.format("cash-out-cmd [sum: %s; password: %s]", sum, PortAdapterUtils.arrayToString(password));
    }
