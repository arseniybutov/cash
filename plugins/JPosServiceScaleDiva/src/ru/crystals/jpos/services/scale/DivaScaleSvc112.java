...
public class DivaScaleSvc112 implements ScaleService112, JposConst {
	...
	protected DivaScaleConfig dsConfig;
	protected DivaConnector dConnector;

	public DivaScaleSvc112() {
		_fAutoDisable = false;
		...
		jposEntry = null;
//		dsConfig = new ScaleConfig("COM1", 9600);
		dsConfig = new DivaScaleConfig();
		dConnector = null;
	}
...
//		System.out.println("---{ READ_WEIGHT }---");
		if (!_fOpened) throw new JposException(101, "Service is not open");
		if (!_fClaimed) throw new JposException(103, "Device is not claimed");

		if (!_fdeviceEnabled) throw new JposException(105, "Device is not enabled");

		try {
			result[0] = dConnector.getWeight(msTimeOut);
...
//		System.out.println("---{ OPEN }---");
		if (_fOpened) throw new JposException(106, "Service is already open");
		jposEntry = JposServiceLoader.getManager().getEntryRegistry().getJposEntry(logicalName);
		if (jposEntry == null) throw new JposException(109, "Logical device could not be found");
		...
		jpos.config.JposEntry.Prop prop = jposEntry.getProp("productDescription");
		if (prop != null) _strPhysicalDesc = prop.getValueAsString();
		if (_strPhysicalDesc == "")	_strPhysicalDesc = "Digi Serial Scale";
		_strPhysicalName = "";

		prop = jposEntry.getProp("productName");
		if (prop != null) _strPhysicalName = prop.getValueAsString();
		if (_strPhysicalName == "") _strPhysicalName = "Digi Scale";

		prop = jposEntry.getProp("port");
		if (prop != null) {
			String portName = prop.getValueAsString();
			if (portName.length() > 0)
				dsConfig.setPortName(prop.getValueAsString());
		}
		...
		if (prop != null) {
			...
			if (parity.length() > 0)
				dsConfig.setParity(parity);
		}
		
		dConnector = new DivaConnector();
...
//		System.out.println("---{ CLAIM }---");
		...
		try {
			dConnector.open(dsConfig.getPortName(), dsConfig.getBaudRate(), dsConfig.getDataBits(), dsConfig.getStopBits(), dsConfig.getParity());
		} catch (JposException je) {
			throw je;
		} catch (Exception e) {
			e.printStackTrace();
