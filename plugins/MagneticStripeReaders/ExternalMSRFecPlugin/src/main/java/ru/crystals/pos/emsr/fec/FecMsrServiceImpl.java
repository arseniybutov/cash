package ru.crystals.pos.emsr.fec;
...
public class FecMsrServiceImpl extends ExternalMSRPluginImpl {

    private static final Logger LOG = LoggerFactory.getLogger(FecMsrServiceImpl.class);

    private static final String PROVIDER = "fec";
    ...
    public FecMsrServiceImpl() {
        this(null);
    }

    FecMsrServiceImpl(TrackProcessor trackProcessor) {
    ...
    public String[] getTracks(List<Integer> scanCodeList) {
        ...
        LOG.info("Fec-MSR scanned: " + scanCodeList);

        if (scanCodeList == null || scanCodeList.size() < 2) {
            return new String[4];
        }
    ...
    protected String[] getRawTracks(List<Integer> scanCodeList, MSRSentinels msrSentinels) {
        return MsrProcessor.getTracks(scanCodeList, this);
    }
}