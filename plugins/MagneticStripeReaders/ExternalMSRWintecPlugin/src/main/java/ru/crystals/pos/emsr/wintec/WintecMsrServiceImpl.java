...
public class WintecMsrServiceImpl extends ExternalMSRPluginImpl {

    private static final String PROVIDER = "wintec";
    ...
    public WintecMsrServiceImpl() {
        super(PROVIDER);
    }

    public WintecMsrServiceImpl(TrackProcessor trackProcessor) {
    ...
    public String[] getTracks(List<Integer> scanCodeList) {
        ...
        for (int i = 0; i < rawTracks.length; i++) {
            ...
            if (StringUtils.isEmpty(track) || track.equals(";")) {
                track = null;
            } else {
                // последний символ дублируется
