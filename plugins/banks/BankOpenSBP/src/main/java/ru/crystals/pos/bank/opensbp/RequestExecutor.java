...
public class RequestExecutor {
    ...
    private final RestTemplate restTemplateFormData;
    ...
    public RequestExecutor(SBPProviderConfig config) {
        ...
        restTemplateFormData = new RestTemplate(factory);
        restTemplateFormData.setInterceptors(Collections.singletonList(new RequestInterceptor()));
    }
    ...
    public <T, R> T executeRequestFormData(String requestPath, HttpMethod httpMethod, HttpEntity<R> httpEntity, Class<T> responseType,
                                   Object... variables) {
        ResponseEntity<T> response = restTemplateFormData.postForEntity(config.getUrl() + requestPath, httpEntity, responseType);
        return response.getBody();
    }

    public <T, R> ResponseWithHttpStatus<T> executeRequestWithHttpStatus(String requestPath, HttpMethod httpMethod, HttpEntity<R> httpEntity, Class<T> responseType,
                                                                         Object... variables) {
        ResponseEntity<T> response = restTemplate.exchange(config.getUrl() + requestPath, httpMethod, httpEntity, responseType, variables);

        return new ResponseWithHttpStatus<>(response.getBody(), response.getStatusCode());
    }
    ...
    public RestTemplate getRestTemplateFormData() {
        return restTemplateFormData;
    }
}
