package ru.crystals.pos.scale.mt.divap5;
...
public class MtDivaP5ScaleServiceImpl extends AbstractScalePluginImpl {

    private static final Logger LOG = LoggerFactory.getLogger(MtDivaP5ScaleServiceImpl.class);

    private static final String PROVIDER_NAME = "mt_divap5";
    ...
    private void configureJpos() {
        ...
        jposEntry.addProperty(JposEntry.SI_FACTORY_CLASS_PROP_NAME, "ru.crystals.jpos.services.scale.DivaScaleSvc112Factory");

        jposEntry.addProperty("port", config.getPort());
        jposEntry.addProperty("baudRate", config.getBaudRate());
        jposEntry.addProperty("dataBits", config.getDataBits());
        jposEntry.addProperty("stopBits", config.getStopBits());
        jposEntry.addProperty("parity", config.getParity());
